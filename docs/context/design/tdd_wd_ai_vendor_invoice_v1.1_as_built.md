...
Statement / Batch Wizard
  ...
  Native PDF / Markdown / Rendered Images
        |
        v
  Provider Adapter
        |
...
5. Input Mode 只负责 PDF/Markdown/Image transport，不选择 Profile。
...
|---|---|
|`description`|业务记录描述|
|`quantity`/`price_unit`/`amount`|金额基础|
|`tax_rate`|该业务行税率|
|`tax_amount`|该业务行税额|
...
|Input Mode|Native PDF / Markdown / Rendered Images transport|
...
parsed -> bill_generated
```

`awaiting_review` 和 `bill_generated` 保留历史兼容语义。Statement Draft
才允许自动 Projection。
...
Vendor Bill 创建不得直接读取 Canonical、Mapping 或 Provider response
补充业务字段。已 Confirm、Bill Created 或 Cancelled 的 Statement 不得被
AI Projection 覆盖。
...
- Prompt Composition：Native PDF、Markdown、Vision；
...
|Batch Supplier = UPS|Supplier 显式传递到 Batch/Statement/Attempt|
